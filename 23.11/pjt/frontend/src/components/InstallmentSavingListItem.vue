<template>
  <RouterLink class="product-card" :to="{ name: 'interestDetail', params: { fin_prdt_cd: installment_saving.fin_prdt_cd }}">
    <img :src="store.bankLogos[installment_saving.kor_co_nm]" alt="">
    <div class="product-card-content">
      <button v-if="isRegistered" class="registered">가입한 상품</button>
      <div class="bank-name">{{ installment_saving.kor_co_nm }}</div>
      <div class="product-name">{{ clearName }}</div>
      <div class="join-way">{{ installment_saving.join_way }}</div>
    </div>
  </RouterLink>
  <RouterView />
</template>
...
<script setup>
  ...
  const isRegistered = computed(() => {
    ...
    for (const registeredInstallmentSaving of store.registeredInstallmentSavingList) {
      if (registeredInstallmentSaving === props.installment_saving.fin_prdt_cd) {
        inList.value = true
        break
      }
    }
  ...
  const clearName = computed(() => {
    return props.installment_saving.fin_prdt_nm.replace(/\s*\([^)]*\)\s*/gs, '')
  })
</script>
